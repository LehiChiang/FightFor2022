package leetcode.editor.cn;//一个班级里有 n 个学生，编号为 0 到 n - 1 。每个学生会依次回答问题，编号为 0 的学生先回答，然后是编号为 1 的学生，以此类推，直到编号为
...
class chalkReplacerSolution {
    public int chalkReplacer(int[] chalk, int k) {
        int n = chalk.length - 1;
        for (int i = 0; i < chalk.length; i++) {
            chalk[i] = i == 0 ? chalk[i] : chalk[i] + chalk[i - 1];
            if (chalk[i] > k)
                return i;
        }
        int sum = chalk[n];
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.chalkReplacer(new int[]{3,7,8,10}, 9));
    }
}
