package leetcode.editor.cn;//nums1 中数字 x 的 下一个更大元素 是指 x 在 nums2 中对应位置 右侧 的 第一个 比 x 大的元素。
//
...
// Related Topics 栈 数组 哈希表 单调栈 👍 621 👎 0
    ...
    public int[] nextGreaterElement(int[] nums1, int[] nums2) {
        ...
        for (int i = 0; i < nums2.length; i++) {
            while (!stack.isEmpty() && nums2[stack.peek()] < nums2[i]) {
                rightMax[stack.peek()] = nums2[i];
                stack.pop();
            }
            stack.push(i);
        }
        for (int i = 0; i < nums1.length; i++)
            res[i] = rightMax[map.get(nums1[i])];
        return res;
    }
