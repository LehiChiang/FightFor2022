...
```
 ...
 泛型类可以扩展或事项其他的泛型类。例如，`ArrayList<T>`类实现了`List<T>`接口。这意味着，一个`ArrayList<Manager>`可以转换为一个`List<Manager>`。但是`ArrayList<Manager>`不是一个`ArrayList<Employee>`或`List<Employee>`。
